...
library(dplyr);library(stringr);library(ggplot2);library(tidyverse);library(knitr);library(kableExtra);library(RColorBrewer)
...
deptos_ordenados <- Datos_ImputR %>%
  ...
  pull(departamento)
paleta_rojos <- colorRampPalette(brewer.pal(9, "Reds"))(length(deptos_ordenados))
datos_para_hc <- Datos_ImputR %>%
  mutate(departamento = factor(departamento, levels = deptos_ordenados))
p6 <- hchart(
  datos_para_hc,
  "boxplot",
  hcaes(x = departamento, y = rendimiento, group = departamento)
) %>%
  hc_colors(paleta_rojos) %>%
  hc_title(
    text = "<b style='color:#8B0000; font-size:18px;'>4.2. Comportamiento del Rendimiento del Maíz por Departamento</b>"
  ) %>%
  hc_xAxis(title = list(text = "Departamento")) %>%
  hc_yAxis(title = list(text = "Rendimiento (Toneladas/Hectárea)")) %>%
